...
def main():
    ...
    # Push trace to Victoria Metrics.
    data = []
    for i, row in metrics.iterrows():
        time_ms = row["time"]
        for name in names:
            value = row[name].item()
            entry = f'rmsjob_trace{{jobid="{args.job}",traceid="{args.trace}",marker="{name}"}} {value} {time_ms}'
            data.append(entry)

    victoria_url = f"http://{args.endpoint}:8428/api/v1/import/prometheus"
    push_to_victoria_metrics(data, victoria_url)
